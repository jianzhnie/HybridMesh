...
Vendored from torchtitan ``experiments/transformers_modeling_backend/hf_sharding.py``.
Import lines were rewritten to hpmesh-relative ones, and the flex kernel is now
a plain ``nn.Module`` (hpmesh has no ``Module.Config`` protocol) so this file no
longer pulls in torchtitan's ``Configurable``. The sharding logic itself is
unchanged.
"""
...
def _hf_activation_placement(
    *,
    ...
    # Keep the default in the signature (upstream spelling, copied verbatim);
    # it is the seq-dim shard an HF (batch, sequence, hidden) tensor carries.
    cp: spmd.PerMeshAxisSpmdType = spmd.S(1),  # noqa: B008
) -> SpmdType:
...
def _attach_flex_kernel(attn: nn.Module) -> None:
    ...
    attn._titan_flex_kernel = HFFlexKernel(
