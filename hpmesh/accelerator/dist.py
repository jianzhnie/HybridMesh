...
def _digit_version(version: str) -> tuple:
    ...
    segment contributes its leading digits (``'2.2.2+cu121'`` -> ``(2, 2,
    2)``), so plain tuple comparison orders releases correctly.
    """
    parts = []
...
def all_reduce(data: Tensor,
    ...
    Examples:
        >>> import torch
        >>> from hpmesh.accelerator import dist
    ...
    if world_size > 1:
        ...
        split_tensors = [
            x.reshape(shape) for x, shape in zip(
                torch.split(flatten_tensor, tensor_sizes),
                tensor_shapes,
                strict=True)
        ]

        for k, v in zip(keys, split_tensors, strict=True):
            data[k] = v
...
def all_reduce_params(params: list | Generator[torch.Tensor, None, None],
                      coalesce: bool = True,
                      bucket_size_mb: int = -1,
                      op: str = 'sum',
                      group: ProcessGroup | None = None) -> None:
