...
rank. The reference is the same model trained single-rank on the union of both
batches with AC OFF. Step-0 gradients, the loss trajectory, and final
parameters must match within fp32 sum-ordering noise: AC is a memory trade,
not a numerics change (``preserve_rng_state=True`` makes the backward-time
recompute see the forward's RNG state).
...
def main() -> None:
    ...
    # -- distributed run, AC ON, through the real entry points ----------------
    model = apply_ac(_build_model(), "full")
